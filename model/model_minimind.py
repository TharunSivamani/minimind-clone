...
class MiniMindConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        ####################################################
        # Here are the specific configurations for MOE.
        # When use_moe is false, the following settings are invalid.
        ####################################################
        ...
        self.aux_loss_alpha = aux_loss_alpha  # Alpha parameter for the auxiliary loss
        self.seq_aux = (
            seq_aux  # Whether to compute auxiliary loss at the sequence level
        )
        self.norm_topk_prob = (
...
def precompute_freqs_cis(
    ...
    """
    freqs → a vector of size (dim/2) containing frequency coefficients
    attn_factor = 1.0

    1. torch.arange(0, dim, 2) -> [0, 2, 4, ..., dim - 2] -> [: (dim // 2)] (keep them)
    2. idx = torch.arange(...).float() / dim -> 0/d, 2/d, 4/d
    3. rope_base ** (idx) -> (10^6) ** (i/d)
    4. freqs = 1.0 / (rope_base ** idx) -> take reciprocal freq-i = (10^6) ** (-i/d)
    ...
    def __init__(self, config: MiniMindConfig):
        ...
        if config.intermediate_size is None:
            intermediate_size = int(
                config.hidden_size * 8 / 3
            )  # (512 * 8) / 3 = int(1365.33) = 1365
            config.intermediate_size = 64 * (
                (intermediate_size + 64 - 1) // 64
            )  # (64 * (1365 + 64 - 1) // 64) = 1426
